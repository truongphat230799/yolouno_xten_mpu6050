...
class AngleSensor(object):
    ...
    async def calibrate_mag(self, stopfunc):
        res = await self.read_imu()
        mag = res[2]
    ...
    async def calibrate(self, samples=500):
        print('Angle sensor start caliberation')
        res = await self.read_imu()
        calib_mag = True
    ...
    async def read_imu(self):
        ...
        if len(imu_data) == 2 or (self.expect_ts and len(imu_data) == 3):
            return imu_data[0].xyz, imu_data[1].xyz
        else:
            return imu_data[0].xyz, imu_data[1].xyz, imu_data[2].xyz
    ...
    async def _update_nomag(self, slow_platform):
        while True:
            ...
            if self.expect_ts:
                accel, gyro, ts = await self.read_imu()
            else:
                accel, gyro = await self.read_imu()
                ts = None
    ...
    async def _update_mag(self, slow_platform):
        while True:
            ...
            if self.expect_ts:
                accel, gyro, mag, ts = await self.read_imu()
            else:
                accel, gyro, mag = await self.read_imu()
                ts = None
